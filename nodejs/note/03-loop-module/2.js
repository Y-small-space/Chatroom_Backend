// Node概念：runtime 运行时，可以让我们js运行在服务端口（底层用的是v8引擎）
// Node的特点：非阻塞I/O（异步的非阻塞）事件驱动（支持我们的js语法，不支持DOM和BOM，内置模块）
// Node + 内置模块 + 第三方模块 = node

// 异步的非阻塞，同步阻塞（node中给我们实现的api都支持异步，底层采用的是多线程模型），异步阻塞（没有意义），同步非阻塞

// 线程池的概念 默认创建10个线程
// 多线程 可以同时执行多个任务 给每个请求分配一个线程 优点就是可以并行 缺点浪费资源 适合cpu密集型（压缩、加密）
// 多线程的缺点 线程间的通信比较麻烦 多个线程如果冲突了 锁 开辟内存来管理线程
// 单线程 只有一个 节约内存 不适合 cpu 密集型的操作 适合I/O密集型 缺点就是 容易阻塞
// nginx
// 事件驱动 按照事件的顺序来触发处理逻辑 （事件环）读写操作完成后会放到对应的队列中，等待事件环来
// 触发对应的事件

// nodejs 适合做什么？
// 编写前端工具 工具链  vite rollup webpack gulp 脚手架 （好处是js）
// 为前端服务的后端 中间层 ((client)(bff、格式化、云服务、处理跨域、代理))(server)
// 创建服务器 koa express eggjs nestjs 可以用来开发 服务器
// 聊天、socket.io、即时通讯、爬虫、ssr 都可以在node中去搞

// 前端 最早没有模块化的概念 （seajs requirejs），node中自己实现了一套模块化规范
// 前端比较主流的 esmodule，nodejs支持commonjs -> Module

// 模块化规范解决了哪些问题？
// 命名冲突 代码隔离上的问题 方便代码复用 提升了代码的扩展性
// 按照每个文件来划分模块，一个文件就是一个独立的模块（在每个文件的外面都包装了一个函数）

// commonjs规范 规范的目的是 定义 如何导出一个模块 如果让别人引用我 做到多个文件之间共享逻辑的处理

// 1. 每个js文件都是一个模块 
// 2. 如果你想给别人用你的逻辑 用 module.exports 导出
